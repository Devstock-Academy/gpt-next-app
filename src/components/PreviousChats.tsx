...
export default async function PreviousChats() {
  ...
  return (
    <div>
      {chats.length > 0 && (
        <>
          <div className='text-2xl font-bold'>Twoje poprzednie rozmowy</div>
          <div className='grid grid-cols-1 md:grid-cols-2'>
            {chats.map((chat) => (
              <div key={chat.id} className='m-1 border-2 rounded-xl'>
                <Link
                  href={`/chats/${chat.id}`}
                  className='text-lg line-clamp-1 px-5 py-2 text-white bg-zinc-800 rounded-t-lg'
                >
                  {chat.name}
                </Link>
                <div className='p-3'>
